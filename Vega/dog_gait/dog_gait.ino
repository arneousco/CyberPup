...
void leg(double hlen, double Y, double pitch, int side, int front){
  pitch = pitch*(PI/180);
        ...
        if (side == 1 && front == 1){
          ...
          double s7x = servo_Map(shoulderAngleDeg,90,0,0,90);
          servo3.write(s7x); //shoulder angle
        }
        else if (side == 1 && front == 0){
          servo5.write(kneeAngleDeg+30);
          ...
          servo8.write(s11x);
          double s8x = servo_Map(shoulderAngleDeg,90,0,180,90);
          servo4.write(s8x);
        }
        else if (side == 0 && front == 0){
          double s6x = servo_Map(kneeAngleDeg,0,180,180,0);
          servo6.write(s6x);
          double s10x = servo_Map(shoulderAngleDeg,90,0,180,90);
          servo2.write(s10x);
        }
}
